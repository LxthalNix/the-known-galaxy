...
export const filterGroups = [
  ...
  { key: 'type' as const, label: 'Event type', options: eventTypes.map((type) => ({ id: type, label: type })) },
  { key: 'importance' as const, label: 'Significance', options: ['major', 'standard', 'minor'].map((id) => ({ id, label: id })) },
];

export function FilterPanel({ open, onClose, filters, onChange, visible, total }: { open: boolean; onClose: () => void; filters: Filters; onChange: (filters: Filters) => void; visible: number; total: number }) {
