...
import { formDefinition, referenceData, eraLabel, generatedFiles, site } from './lore-form.mjs';
...
export function parseForm(body, records) {
  ...
  // Require all structural headings, including optional fields. Older forms need resubmission.
  for (const field of fields) if (!body.includes(`### ${field.attributes.label}\n`)) errors.push(`Missing current form field: ${field.attributes.label}. Use the current form.`);
...
export function validateSubmission(issue, records) {
  ...
  for (const ref of related) if (!ref.slug || ref.slug === slug || !records.some((r) => !r.data.draft && r.data.slug === ref.slug)) errors.push(`Related events: “${ref.input}” must be another published event's exact slug or full website link; display titles are not accepted.`);
  ...
  const data = {
    ...
    year: integer(v.year, undefined, 'Year', true), calendar: v.calendar,
  ...
  return { valid: !errors.length, errors: [...new Set(errors)], warnings, data: result.success ? result.data : data, article: v.article, imageUrl, existing, bodyHash: bodySha(issue.body), values: v };
}
