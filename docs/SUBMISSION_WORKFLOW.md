...
4. Submit a genuine lore proposal using the new form. Issues using older original headings must be copied into the current form. Complete issues using the previous event form remain compatible when only the new optional gallery/account fields are absent; the former future-era label is also accepted.
...
1. Open the form from **Issues → New issue → Submit or update a lore event**.
...
3. Complete the whole proposed article and metadata. For corrections, choose **Update an existing event** and supply its link. Leave irrelevant optional lists blank. Upload the selected main image in its dedicated field. Optional gallery slots have their own uploads, required alt text, optional captions and permission evidence. Optional Jedi/Sith articles have explicit Keep, Add or replace, and Remove actions. Supporting files stay in the issue.
4. Create the issue. Wait for the workflow's feedback comment. If fields are invalid, edit the issue body to correct them; checks run again. GitHub converts the form to a Markdown issue body, so edits do not reopen the original dropdown form. Keep every existing `### Field label` heading, even for empty optional fields. The newly added optional account/gallery headings may be absent in older complete issues. Empty optional values can be `_No response_`.
...
3. Preparation imports and decodes the selected main/gallery images, optimizes them to WebP, writes metadata and canonical/faction article files, regenerates the form/reference, runs checks/tests, and builds previews. The main image needs source/permission evidence and alt text. Still PNG, JPEG, and WebP files are supported, up to 10 MiB and 40 million pixels; SVG, GIF, documents, videos, and outside hosts remain supporting evidence only. Kept image paths and slugs survive corrections. Replacing/removing imagery does not delete the old asset automatically.
